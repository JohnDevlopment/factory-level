tool
extends "res://scenes/components/commands/enemy_commands/EnemyCommand.gd"
...
func _get_configuration_warning() -> String:
	...
	if not door:
		...
		if not _check_valid_door(door):
			warning += "The node '%s' is not a PoweredDoor; if it was, it would have the 'do_animation' method." % powered_door
